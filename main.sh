...
if [[ "$DOWNLOAD" != "0" ]]; then
  rm -rf $src
  git clone --depth=1 --recursive https://github.com/wolfram77/$src -b lowmem-communities--louvain
  cd $src
  git checkout lowmem-communities--louvain
fi
