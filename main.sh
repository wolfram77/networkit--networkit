...
# Convert graph to edgelist, run Networkit PageRank, and clean up
runNetworkit() {
  stdbuf --output=L printf "Converting $1 to $1.el ...\n" | tee -a "$out"
  lines="$(node process.js header-lines "$1")"
  tail -n +$((lines+1)) "$1" > "$1.el"
  stdbuf --output=L python3 main.py "$1.el"          2>&1 | tee -a "$out"
  stdbuf --output=L printf "\n\n"                         | tee -a "$out"
  rm -rf "$1.el"
}

# Run Networkit PageRank on all graphs
runAll() {
...
# Run NetworKit PageRank 5 times for each graph
for i in {1..5}; do
  runAll
done
